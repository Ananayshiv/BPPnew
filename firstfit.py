...
def visualize_bins(bins: List[Bin]):
    ...
    grid_size = int(np.ceil(np.sqrt(len(bins))))
    max_dim = max(max(bin.size) for bin in bins) if bins else 1
    spacing = max_dim * 1.2
    
    for bin_idx, bin in enumerate(bins):
        # Calculate grid position
        row = bin_idx // grid_size
        col = bin_idx % grid_size
        offset_x = col * spacing
        offset_y = row * spacing
        
        # Draw bin container as wireframe
        bin_lines = [
            # Bottom rectangle
            [(0, 0, 0), (bin.size[0], 0, 0)],
            [(bin.size[0], 0, 0), (bin.size[0], bin.size[1], 0)],
            [(bin.size[0], bin.size[1], 0), (0, bin.size[1], 0)],
            [(0, bin.size[1], 0), (0, 0, 0)],
            
            # Top rectangle
            [(0, 0, bin.size[2]), (bin.size[0], 0, bin.size[2])],
            [(bin.size[0], 0, bin.size[2]), (bin.size[0], bin.size[1], bin.size[2])],
            [(bin.size[0], bin.size[1], bin.size[2]), (0, bin.size[1], bin.size[2])],
            [(0, bin.size[1], bin.size[2]), (0, 0, bin.size[2])],
            
            # Vertical lines
            [(0, 0, 0), (0, 0, bin.size[2])],
            [(bin.size[0], 0, 0), (bin.size[0], 0, bin.size[2])],
            [(bin.size[0], bin.size[1], 0), (bin.size[0], bin.size[1], bin.size[2])],
            [(0, bin.size[1], 0), (0, bin.size[1], bin.size[2])]
        ]
        
        for line in bin_lines:
            fig.add_trace(go.Scatter3d(
                x=[line[0][0] + offset_x, line[1][0] + offset_x],
                y=[line[0][1] + offset_y, line[1][1] + offset_y],
                z=[line[0][2], line[1][2]],
                mode='lines',
        ...
        # Add bin label
        fig.add_trace(go.Scatter3d(
            x=[offset_x + bin.size[0]/2],
            y=[offset_y + bin.size[1]/2],
            z=[bin.size[2] * 1.05],
            mode='text',
        ...
        # Draw items in bin as solid boxes
        for item_idx, item in enumerate(bin.items):
            ...
            # Add each face as a surface
            for face in faces:
                # Apply bin offset
                x_vals = [point[0] + offset_x for point in face]
                y_vals = [point[1] + offset_y for point in face]
                z_vals = [point[2] for point in face]
    ...
    # Configure the layout to match the reference image
    max_grid = grid_size * spacing
    fig.update_layout(
        scene=dict(
            xaxis=dict(
                title='X', 
        ...
        title=dict(
            text=f'3D Bin Packing - {len(bins)} Bins Used',
...
def visualize_single_bin(bin: Bin, bin_label: str = "Bin 1"):
    """
    Visualize a single bin with packed items in 3D
    
    Args:
        bin: Bin object containing placed items
        bin_label: Text label to display above the bin
    """
    if not bin.items:
        print("No items to visualize in this bin")
        return
    ...
    for item_idx, item in enumerate(bin.items):
        ...
        # Define faces (each face as a polygon)
        faces = [
            # Bottom face
            [vertices[0], vertices[1], vertices[2], vertices[3]],
            # Top face
        ...
        # Add each face as a surface
        for face in faces:
            x_vals = [point[0] for point in face]
            y_vals = [point[1] for point in face]
            z_vals = [point[2] for point in face]
    ...
    fig.add_trace(go.Scatter3d(
        ...
        z=[0, 0, 0, bin.size[2]],
        mode='text',
        text=['(0,0,0)', f'({bin.size[0]:.1f},0,0)', f'(0,{bin.size[1]:.1f},0)', f'(0,0,{bin.size[2]:.1f})'],
    ...
    fig.update_layout(
        scene=dict(
            ...
            aspectmode='manual',
            aspectratio=dict(x=1, y=bin.size[1]/bin.size[0], z=bin.size[2]/bin.size[0]),
            camera=dict(
                up=dict(x=0, y=0, z=1),
                eye=dict(x=1.5, y=1.5, z=0.8)
            )
        ...
        title=dict(
            text=f'3D Bin Packing: {bin_label}',
